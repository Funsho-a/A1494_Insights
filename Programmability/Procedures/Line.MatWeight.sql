...
BEGIN
	SET NOCOUNT ON
		...
		BEGIN
			...
			[Full Date], [MonoFaceDMC],	[Monolith],	[Variant],	[Mat Weight (g)],	[Mat Weight LSL],	[Mat Weight USL],	[Mat Weight Result]
			FROM	[CTE] c			
			...
			AND		[Variant] =			CASE	WHEN @Variant IS NULL 
												THEN [Variant]
												ELSE @Variant END 
			AND		[Mat Weight Result]  =	CASE	WHEN @MatWeightResult IS NULL 
												THEN [Mat Weight Result] 
												ELSE @MatWeightResult END										
