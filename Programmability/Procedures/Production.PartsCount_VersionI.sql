...
GO
CREATE PROCEDURE  [Production].[PartsCount_VersionI] 
...
---------------------------------------------------------------------------------------------------
	...
	DECLARE @HDateTo DATETIME = @HDateFrom +' 23:59:59'
		...
		SET hpc.MonoCount = ho.MonoCount ,
			hpc.MonoPassCount = ho.MonoPassCount,
			hpc.MonoFailCount = ho.MonoFailCount 
		FROM Mono_Count ho 
		...
		SET		
				...
				[d].MonoPassCount = [p].MonoPassCount,
				[d].MonoFailCount = [p].MonoFailCount
		FROM	@HourlyData [d]	
		INNER JOIN #HourlyPartsCount [p] ON  [p].StartTime = [d].StartTime 
		...
		SELECT	FORMAT(StartTime,'yyyy-MM-dd HH:mm') [Date],				
				...
				ISNULL(MonoPassCount, 0) [MonoPassCount],
				ISNULL(MonoFailCount, 0) [MonoFailCount]
		FROM	@HourlyData
		ELSE
	...
	BEGIN
		...
		---------------------------------------------------------------------------------------------------
			...
			UPDATE hpc
			SET hpc.MonoCount = da.MonoCount,
				hpc.MonoPassCount = da.MonoPassCount,
				hpc.MonoFailCount = da.MonoFailCount 
			FROM Mono_Count da 
			INNER JOIN #DailyPartCount hpc on da.[Timestamp] = hpc.[Timestamp]			
...
---------------------------------------------------------------------------------------------------
	...
	BEGIN			
		...
		;WITH Mono_Count	AS 
		(					
			SELECT	COUNT(MonoFaceDMC) [MonoCount], c.[Timestamp], SUM(c.MonoPassCount) MonoPassCount, SUM(c.MonoFailCount) MonoFailCount
			FROM	(
						...
						SELECT   Distinct MonoFaceDMC,
							...
							WHERE	(CAST([DateTimeCompleted] AS DATE) >= @StartDate OR @StartDate IS NULL)
									...
									AND (CAST([DateTimeCompleted] AS DATE) = @ThisDay OR @ThisDay IS NULL) 
									AND [*Variant Name] = CASE	WHEN @Variant IS NULL THEN [*Variant Name]	ELSE @Variant END
							GROUP BY [DateTimeCompleted],MonoFaceDMC
						) c		
			RIGHT JOIN @WeeklyDates [d] ON [c].[Timestamp] BETWEEN [d].[StartDate] AND [d].[EndDate] 
			GROUP	BY [Timestamp]
		)		
		UPDATE wpc
		SET wpc.MonoCount = we.MonoCount,
			wpc.MonoPassCount = we.MonoPassCount,
			wpc.MonoFailCount = we.MonoFailCount 
		FROM Mono_Count we 
		INNER JOIN #WeeklyPartCount wpc on wpc.Timestamp = we.Timestamp 
			...
			FROM	@WeeklyDates [d] INNER JOIN (
						...
						SUM([p].MonoCount) [MonoCount], SUM([p].MonoPassCount) [MonoPassCount],SUM([p].MonoFailCount) [MonoFailCount]
...
---------------------------------------------------------------------------------------------------
	...
	BEGIN 	
		...
		SET mpc.MonoCount = mo.MonoCount ,
			mpc.MonoPassCount = mo.MonoPassCount ,
			mpc.MonoFailCount = mo.MonoFailCount
		FROM Mono_Count mo 
		...
		FROM	@MonthlyDates [d] 
		INNER JOIN (
					SELECT	[Date], SUM([p].[PartsCount]) [PartsCount],SUM([p].[Pass Count])[Pass Count] , SUM([p].[Fail Count]) [Fail Count], AVG([p].[Avg Cycle Time (s)]) [Avg Cycle Time (s)],	sum([p].[InProgress Count]) [InProgress Count],  SUM([p].MonoCount) [MonoCount],  SUM([p].MonoPassCount) [MonoPassCount],  SUM([p].MonoFailCount) [MonoFailCount]
