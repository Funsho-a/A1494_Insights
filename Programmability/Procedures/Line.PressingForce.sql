...
BEGIN
	SET NOCOUNT ON
		...
		--IF(@Monolith IS NULL AND @Variant IS NULL AND @TopRecords = 999999 AND @PressingResult IS NULL)
		--	SET @TopRecords = 0 
			...
			;WITH [CTE]	AS (
				SELECT	 FORMAT([DateTimeCompleted],'yyyy-MM-dd HH:mm') AS [Date],
					...
					[*Variant Name] AS [Variant],
						CASE	WHEN (ISNULL( CAST([Pressing Force (N)] AS FLOAT), 0)) = 0 THEN 0 ELSE  CAST([Pressing Force (N)] AS FLOAT) END AS [Pressing Force (N)],
						CASE	WHEN (ISNULL( CAST([*Pressing Force Min (N)] AS FLOAT), 0)) = 0 THEN 0 ELSE  CAST([*Pressing Force Min (N)] AS FLOAT) END AS [Pressing LSL],
						CASE	WHEN (ISNULL( CAST([*Pressing Force Max (N)] AS FLOAT), 0)) = 0 THEN 0 ELSE  CAST([*Pressing Force Max (N)] AS FLOAT) END AS [Pressing USL],
		...
		BEGIN
			...
			[Full Date], [MonoFaceDMC],	[Monolith],	[Variant],	[Pressing Force (N)],	[Pressing LSL],	[Pressing USL],[Pressing Force Result]
			FROM	[CTE] c			
			...
			AND		[Variant] =			CASE	WHEN @Variant IS NULL 
												THEN [Variant]
												ELSE @Variant END 
			AND		[Pressing Force Result]  =	CASE	WHEN @PressingResult IS NULL 
												THEN [Pressing Force Result] 
												ELSE @PressingResult END										
