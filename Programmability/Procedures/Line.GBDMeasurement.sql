...
GO
CREATE PROCEDURE [Line].[GBDMeasurement]
		...
		BEGIN
				...
				SELECT	[DateTimeCompleted] AS [Full Date],
					...
					[*Variant Name] AS [Variant],
						CASE	WHEN (ISNULL( CAST([Comp Crit Overall Avg GBD (g/cm^3)] AS FLOAT), 1)) = 0 THEN 1 ELSE  CAST([Comp Crit Overall Avg GBD (g/cm^3)] AS FLOAT) END AS [Average GBD (g/cm^3)],
						CASE	WHEN (ISNULL( CAST([*Comp Crit Overall Avg GBD Lower Tol (g/cm^3)] AS FLOAT), 1)) = 0 THEN 1 ELSE  CAST([*Comp Crit Overall Avg GBD Lower Tol (g/cm^3)] AS FLOAT) END AS [GBD LSL],
						CASE	WHEN (ISNULL( CAST([*Comp Crit Overall Avg GBD Upper Tol (g/cm^3)] AS FLOAT), 1)) = 0 THEN 1 ELSE CAST([*Comp Crit Overall Avg GBD Upper Tol (g/cm^3)] AS FLOAT) END AS [GBD USL],						CASE	WHEN [Total GBD MM Result] = 'PASS' THEN 'OK'
								WHEN [Mono MM Result] = 'PASS' AND [Total Pressing Result] = 'PASS' AND [Total GBD MM Result] = 'FAIL' THEN  'NOK' 
								WHEN [Total GBD MM Result] IS NULL THEN 'NOK' END AS [GBD Result]
				FROM    [A1494].LineResults.DenormalisedMegaView 
				WHERE	[*Variant Name] IS NOT NULL							)
			...
			[Full Date], [MonoFaceDMC],	[Monolith],	[Variant],	[Average GBD (g/cm^3)],	[GBD LSL],	[GBD USL],[GBD Result]
			FROM	[CTE] c			
