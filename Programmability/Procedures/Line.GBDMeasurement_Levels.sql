...
BEGIN
	SET NOCOUNT ON
		...
		--IF(@Monolith IS NULL AND @Variant IS NULL AND @TopRecords = 999999 AND @GBDResult IS NULL)
		--	SET @TopRecords = 0 
			...
			;WITH [CTE]	AS (
				SELECT	 FORMAT([DateTimeCompleted],'yyyy-MM-dd HH:mm') AS [Date],
					...
					[*Variant Name] AS [Variant],
						CASE	WHEN (ISNULL( CAST([Comp Crit Overall Avg GBD (g/cm^3)] AS FLOAT), 0.0001)) = 0 THEN 1 ELSE CAST([Comp Crit Overall Avg GBD (g/cm^3)] AS FLOAT) END AS [Average GBD (g/cm^3)],
						...
						CASE	WHEN [Total GBD MM Result] = 'PASS' THEN 'OK'
								WHEN [Mono MM Result] = 'PASS' AND [Total Pressing Result] = 'PASS' AND [Total GBD MM Result] = 'FAIL' THEN  'NOK' 
								WHEN [Total GBD MM Result] IS NULL THEN 'NOK' END AS [GBD Result]
		...
		BEGIN
			...
			[Full Date], [MonoFaceDMC],	[Monolith],	[Variant],	[Average GBD (g/cm^3)],	[GBD Average level 1 (g/cm^3)],	[GBD Average level 2 (g/cm^3)],[GBD Average level 3 (g/cm^3)],[GBD Result]
			FROM	[CTE] c			
			...
			AND		[Variant] =			CASE	WHEN @Variant IS NULL 
												THEN [Variant]
												ELSE @Variant END 
			AND		[GBD Result]  =	CASE	WHEN @GBDResult IS NULL 
												THEN [GBD Result] 
												ELSE @GBDResult END										
