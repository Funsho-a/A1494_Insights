...
create PROCEDURE [Production].[Production_CycleTimev2]
			...
			@page			INT = 0,
			@rowsPerPage INT = 200,
			@splitNumber INT = 4 -- Number of splits
AS
BEGIN
	SET NOCOUNT ON
	...
	BEGIN
		...
		WHERE	[Monolith] =		CASE	WHEN @Monolith IS NULL 
				...
				[cycle_category]  =	CASE	WHEN @Cycle IS NULL 
											THEN [cycle_category] 
											ELSE @Cycle END	AND
				(CAST([Date] AS DATE) = @ThisDay OR @ThisDay IS NULL) AND
				c.RowNum > ((@page - 1) * @rowsPerPage * @splitNumber) AND
				c.RowNum <= (@page * @rowsPerPage * @splitNumber)
		...
		(  
			...
			AND c.RowNum > ((@page - 1) * @rowsPerPage * @splitNumber)
            AND c.RowNum <= (@page * @rowsPerPage * @splitNumber)
