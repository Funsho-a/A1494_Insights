...
GO
CREATE PROCEDURE  [Production].[PartsCount_CycleTime_VersionIII]
		...
		@RW_NUM			VARCHAR(2) = NULL
		
AS
BEGIN

	SET NOCOUNT ON
	declare @row_set varchar(50)
	...
	IF @Month = '0'
		SET @Month = NULL
	IF @RW_NUM = '0' or @RW_NUM IS NULL
		SET @row_set = ''
	else
		set @row_set=' Where row_num = ' + @RW_NUM 
	...
	DECLARE @Select_1 VARCHAR(MAX) = ' ;With Master_Data AS 
									(SELECT ISNULL(SUM([PassCount]), 0) [Pass Count],
											...
											ISNULL(SUM([PartsCount]), 0) [PartsCount]',
			@From_PC VARCHAR(MAX) = ' FROM	(
									  SELECT	[DateTimeCompleted] AS [Date],
													...
													avg([Cycle Time (s)]) as cycle_time
			...
			@Select_2 VARCHAR(MAX) = ' SELECT [Date], row_num as Row_Number,[Pass Count], [Fail Count], [PartsCount], cycle_time as [Cycle Time]
									FROM Master_Data hd
									LEFT JOIN 
									(
	...
	BEGIN
		SET @Select_1 = 'INSERT INTO #Dates ([StartTime],[EndTime])
		 ...
		 @Select_1 + ' , CAST(DENSE_RANK() OVER(ORDER BY hpc.StartTime) AS VARCHAR(2)) + '') '' 
						...
						+ '' '' + CAST(DAY(hpc.StartTime) AS VARCHAR(2))
							...
							+ '' '' +  FORMAT(hpc.StartTime, ''HH'') + '':'' + FORMAT(hpc.StartTime, ''mm'')  AS [Date], isnull(round(AVG(cycle_time),2),0) as cycle_time,DENSE_RANK() OVER(ORDER BY hpc.StartTime) as row_num'	
		...
		SET @SQL = @Select_1 + @From_PC + @Join + @Select_2 + @row_set-- ' ORDER BY [Date]'
	...
	SET @Select_1 = '
		 ...
		 @Select_1 + ' , CAST(pc.[Date] AS DATE)	 AS [This Day], round(AVG(cycle_time),2) as cycle_time,DENSE_RANK() OVER(ORDER BY CAST(pc.[Date] AS DATE)) as row_num,
	...
	BEGIN	
		...
		@Select_1 +  ' , CAST(DENSE_RANK() OVER(ORDER BY  dpc.[StartDate]) AS VARCHAR(2)) + '') ''
					...
					DATEPART(wk,COALESCE([StartDate], [EndDate], GETDATE())) AS [This Week Position], round(AVG(cycle_time),2) as cycle_time,DENSE_RANK() OVER(ORDER BY CAST( dpc.[StartDate] AS DATE)) as row_num,
					dpc.[StartDate],
					dpc.[EndDate] '	
		...
		SET @SQL = @Select_1 + @From_PC + @Join + @Select_2  + @row_set+ ' ORDER BY  hd.[This Week Position]'
	...
	BEGIN 	
		...
		@Select_1 +  ' ,  CAST(DENSE_RANK() OVER(ORDER BY  dpc.[StartDate]) AS VARCHAR(2)) + '') '' + CAST(DATENAME(MONTH, [StartDate]) AS VARCHAR(10))  AS Date,
					MONTH(CAST([StartDate] AS DATE)) AS [This Month Position], isnull(round(AVG(cycle_time),2),0) as cycle_time,DENSE_RANK() OVER(ORDER BY CAST( dpc.[StartDate] AS DATE)) as row_num,
					dpc.[StartDate],
					dpc.[EndDate] '	
		...
		SET @SQL = @Select_1 + @From_PC + @Join + @Select_2 + @row_set+ ' ORDER BY  [StartDate]'
