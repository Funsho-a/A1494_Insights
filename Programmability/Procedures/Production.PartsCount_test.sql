...
GO
CREATE PROCEDURE  [Production].[PartsCount_test] 
...
BEGIN
	...
	BEGIN	
		...
		@Select_1 +  ' , ''Week '' + CAST(DATEPART(wk,COALESCE([StartDate], [EndDate], GETDATE())) AS VARCHAR(5)) AS Date,
	...
	BEGIN 	
		...
		FROM	[CTE]	' +
		@Select_1 +  ' , CAST(DATENAME(MONTH, [StartDate]) AS VARCHAR(10)) + '', '' + CAST(YEAR([StartDate]) AS VARCHAR(10))  AS Date,
